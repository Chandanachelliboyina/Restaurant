/**
 * Explicit image imports for production bundling.
 * All images are directly imported to ensure they're included in the build.
 */

// Menu dish images - imported explicitly
import crispycornImage from "@/assets/premium_photo-1680118540055-aa9f6ce1d93d.avif";
import harabharakebabImage from "@/assets/istockphoto-1024552618-612x612.jpg";
import fishamritsariImage from "@/assets/photo-1626253836448-e2376678c191.avif";
import mushroomgaloutiImage from "@/assets/images.avif";
import truffleMushroomImage from "@/assets/premium_photo-1663858366999-aa1ce123a972.avif";
import diavolaImage from "@/assets/photo-1571336350540-8b189c0779f4.avif";
import premiumDiavolaImage from "@/assets/premium_photo-1722686519091-886edbd87f7e.avif";
import wontonSoupImage from "@/assets/premium_photo-1664391950572-bc4b1bdd1268.avif";
import creamOfMushroomImage from "@/assets/premium_photo-1669631647057-3403888e87da.avif";
import spicyChickenBurgerImage from "@/assets/premium_photo-1675252369719-dd52bc69c3df.avif";
import paneerTikkaBurgerImage from "@/assets/istockphoto-1401041513-612x612.webp";
import tandooriBroccoliImage from "@/assets/istockphoto-1469649306-612x612.webp";
import manchurianBallsImage from "@/assets/manchurian-balls.jpg";
import amburStarImage from "@/assets/ambur-star-biryani.jpg";
import crispyCornKernelsImage from "@/assets/crispy-corn.jpg";
import photo_1599487488170 from "@/assets/photo-1599487488170-d11ec9c172f0.avif";
import photo_1590534247854 from "@/assets/photo-1590534247854-e97d5e3feef6.avif";
import istockphoto_1292615374 from "@/assets/istockphoto-1292615374-612x612.webp";
import istockphoto_1431115204 from "@/assets/istockphoto-1431115204-612x612.webp";
import photo_1585032226651 from "@/assets/photo-1585032226651-759b368d7246.avif";
import photo_1568901346375 from "@/assets/photo-1568901346375-23c9450c58cd.avif";
import photo_1555126634 from "@/assets/photo-1555126634-323283e090fa.avif";
import premium_photo_1683619761492 from "@/assets/premium_photo-1683619761492-639240d29bb5.avif";
import photo_1666001120694 from "@/assets/photo-1666001120694-3ebe8fd207be.avif";
import photo_1674622720208 from "@/assets/photo-1674622720208-2a67da6ff4e8.avif";
import photo_1695712641569 from "@/assets/photo-1695712641569-05eee7b37b6d.avif";
import photo_1551881192 from "@/assets/photo-1551881192-002e02ad3d87.avif";
import photo_1588923930957 from "@/assets/photo-1588923930957-81c81fd6262b.avif";
import photo_1736239092894 from "@/assets/photo-1736239092894-c73a7b8b9a39.avif";
import photo_1665594051407 from "@/assets/photo-1665594051407-7385d281ad76.avif";
import photo_1672667509988 from "@/assets/photo-1672667509988-baade9ade083.avif";
import photo_1730112696140 from "@/assets/photo-1730112696140-19e61bb43d36.avif";
import photo_1620256114757 from "@/assets/photo-1620256114757-322387444c16.avif";
import photo_1613844237701 from "@/assets/photo-1613844237701-8f3664fc2eff.avif";
import photo_1628430043175 from "@/assets/photo-1628430043175-0e8820df47c3.avif";
import photo_1643786661490 from "@/assets/photo-1643786661490-966f1877effa.avif";
import photo_1751200884901 from "@/assets/photo-1751200884901-c1c6f43ae1d6.avif";
import photo_1655471264223 from "@/assets/photo-1655471264223-b07ce84d521c.avif";
import photo_1573821663912 from "@/assets/photo-1573821663912-569905455b1c.avif";
import photo_1534308983496 from "@/assets/photo-1534308983496-4fabb1a015ee.avif";
import photo_1628672092908 from "@/assets/photo-1628672092908-c92a710bb3d6.avif";
import photo_1617470703128 from "@/assets/photo-1617470703128-26a0fc9af10f.avif";
import photo_1610440042657 from "@/assets/photo-1610440042657-612c34d95e9f.avif";
import photo_1605345981660 from "@/assets/photo-1605345981660-ab44e036a21d.avif";
import photo_1660715683691 from "@/assets/photo-1660715683691-d1614d1dd361.avif";
import photo_1589302168068 from "@/assets/photo-1589302168068-964664d93dc0.avif";
import photo_1752673508949 from "@/assets/photo-1752673508949-f4aeeaef75f0.avif";
import photo_1697155406055 from "@/assets/photo-1697155406055-2db32d47ca07.avif";
import photo_1757445059337 from "@/assets/photo-1757445059337-23e485483425.avif";
import photo_1691171047462 from "@/assets/photo-1691171047462-66025ecd5efc.avif";
import photo_1644677859249 from "@/assets/photo-1644677859249-e0c8f5b4a74b.avif";
import photo_1696950169706 from "@/assets/photo-1696950169706-d4533c32f81a.avif";
import premium_photo_1669261882028 from "@/assets/premium_photo-1669261882028-c418e8865b2e.avif";
import photo_1545247181 from "@/assets/photo-1545247181-516773cae754.avif";
import photo_1620894580123 from "@/assets/photo-1620894580123-466ad3a0ca06.avif";
import photo_1589647363585 from "@/assets/photo-1589647363585-f4a7d3877b10.avif";
import photo_1595608010652 from "@/assets/photo-1595608010652-d8bf1103a1c5.avif";
import photo_1623689046286 from "@/assets/photo-1623689046286-01d812cc8bad.avif";
import photo_1756821753259 from "@/assets/photo-1756821753259-eb8abcdda9a3.avif";
import photo_1730191843435 from "@/assets/photo-1730191843435-073792ba22bc.avif";
import photo_1603496987351 from "@/assets/photo-1603496987351-f84a3ba5ec85.avif";
import photo_1572815438941 from "@/assets/photo-1572815438941-36e4b94f4dda.avif";
import photo_1632104667384 from "@/assets/photo-1632104667384-06f58cb7ad44.avif";
import photo_1633030318854 from "@/assets/photo-1633030318854-b076ff72770f.avif";
import photo_1645024679624 from "@/assets/photo-1645024679624-e8351ac98f01.avif";
import photo_1691171047312 from "@/assets/photo-1691171047312-d809eccef46d.avif";
import photo_1707592357743 from "@/assets/photo-1707592357743-5e25b277ca36.avif";
import photo_1769690507359 from "@/assets/photo-1769690507359-98ad7820b1bb.avif";
import photo_1776729851079 from "@/assets/photo-1776729851079-686daa8ff9aa.avif";
import premium_photo_1661607229579 from "@/assets/premium_photo-1661607229579-dd06d36302fc.avif";
import premium_photo_1675864532625 from "@/assets/premium_photo-1675864532625-60efd11cde54.avif";
import premium_photo_1680118540055 from "@/assets/premium_photo-1680118540055-aa9f6ce1d93d.avif";
import premium_photo_1694860349260 from "@/assets/premium_photo-1694860349260-a2e6d9cdc6d8.avif";
import premium_photo_1695758787947 from "@/assets/premium_photo-1695758787947-0aff87c1f93a.avif";
import istockphoto_1170374719 from "@/assets/istockphoto-1170374719-612x612.webp";
import istockphoto_1256957996 from "@/assets/istockphoto-1256957996-612x612.webp";
import istockphoto_1258129729 from "@/assets/istockphoto-1258129729-612x612.webp";
import istockphoto_1286704566 from "@/assets/istockphoto-1286704566-612x612.webp";
import istockphoto_1322959597 from "@/assets/istockphoto-1322959597-612x612.webp";
import istockphoto_1328524499 from "@/assets/istockphoto-1328524499-612x612.webp";
import istockphoto_1333127664 from "@/assets/istockphoto-1333127664-612x612.webp";
import istockphoto_1365183162 from "@/assets/istockphoto-1365183162-612x612.webp";
import istockphoto_1397648059 from "@/assets/istockphoto-1397648059-612x612.webp";
import istockphoto_1690939355 from "@/assets/istockphoto-1690939355-612x612.webp";
import istockphoto_482366402 from "@/assets/istockphoto-482366402-612x612.webp";

// Gallery/background images
import bgGalleryImage from "@/assets/bg-gallery.jpg";
import bgContactImage from "@/assets/bg-contact.jpg";
import bgAboutImage from "@/assets/bg-about.jpg";
import bgMenuImage from "@/assets/bg-menu.jpg";
import bgReservationsImage from "@/assets/bg-reservations.jpg";
import interiorImage from "@/assets/interior.jpg";
import heroRestaurantImage from "@/assets/hero-restaurant.jpg";
import chefImage from "@/assets/chef.jpg";

// Dish images
import dish1Image from "@/assets/dish-1.jpg";
import dish2Image from "@/assets/dish-2.jpg";
import dish3Image from "@/assets/dish-3.jpg";
import dish4Image from "@/assets/dish-4.jpg";
import dish5Image from "@/assets/dish-5.jpg";
import dish6Image from "@/assets/dish-6.jpg";

// Gallery images
import gallery1Image from "@/assets/gallery-1.jpg";
import gallery2Image from "@/assets/gallery-2.jpg";
import gallery3Image from "@/assets/gallery-3.jpg";

/**
 * Image map for menu dishes.
 * Maps dish names to their imported image modules.
 */
export const DISH_IMAGES: Record<string, string> = {
  "Paneer Tikka": photo_1666001120694,
  "Crispy Corn Kernels": crispyCornKernelsImage,
  "Chicken Tikka": photo_1599487488170,
  "Hara Bhara Kebab": photo_1674622720208,
  "Fish Amritsari": fishamritsariImage,
  "Mushroom Galouti": mushroomgaloutiImage,
  "Prawn Koliwada": istockphoto_1292615374,
  "Veg Spring Rolls": photo_1695712641569,
  "Chilli Paneer Dry": photo_1551881192,
  "Chicken 65": photo_1588923930957,
  "Aloo Tikki Chaat": photo_1736239092894,
  "Tandoori Broccoli": tandooriBroccoliImage,
  "Sweet Corn Veg Soup": photo_1665594051407,
  "Manchow Soup": photo_1672667509988,
  "Hot & Sour Chicken": photo_1730112696140,
  "Lemon Coriander": photo_1620256114757,
  "Wonton Soup": wontonSoupImage,
  "Mulligatawny": photo_1613844237701,
  "Cream of Mushroom": creamOfMushroomImage,
  "Thukpa Noodle Soup": photo_1555126634,
  "Tom Yum Soup": photo_1628430043175,
  "Vegetable Clear Soup": photo_1643786661490,
  "Margherita Classica": istockphoto_1431115204,
  "Truffle Mushroom": truffleMushroomImage,
  "Diavola": premiumDiavolaImage,
  "Quattro Formaggi": photo_1751200884901,
  "BBQ Chicken": photo_1655471264223,
  "Pepperoni Reserve": photo_1590534247854,
  "Paneer Makhani Pizza": photo_1573821663912,
  "Pesto Verde": photo_1534308983496,
  "Meat Feast": photo_1628672092908,
  "Garden Fiesta": photo_1617470703128,
  "Classic Cheeseburger": photo_1568901346375,
  "Double Smash": premium_photo_1683619761492,
  "BBQ Bacon Burger": photo_1610440042657,
  "Spicy Chicken Burger": spicyChickenBurgerImage,
  "Mushroom Swiss": photo_1605345981660,
  "Paneer Tikka Burger": paneerTikkaBurgerImage,
  "Veggie Beetroot": diavolaImage,
  "Fish Fillet Burger": photo_1645024679624,
  "Aloo Tikki Burger": photo_1660715683691,
  "Hyderabadi Chicken Biryani": photo_1589302168068,
  "Lucknowi Mutton Biryani": photo_1752673508949,
  "Veg Dum Biryani": photo_1697155406055,
  "Prawn Biryani": photo_1757445059337,
  "Paneer Biryani": photo_1691171047462,
  "Egg Biryani": photo_1644677859249,
  "Kolkata Biryani": photo_1696950169706,
  "Malabar Chicken Biryani": istockphoto_1333127664,
  "Mushroom Biryani": premium_photo_1669261882028,
  "Ambur Star Biryani": amburStarImage,
  "Chilli Chicken Dry": premium_photo_1675864532625,
  "Veg Hakka Noodles": photo_1589302168068,
  "Kung Pao Chicken": photo_1589302168068,
  "Schezwan Fried Rice": photo_1589302168068,
  "Manchurian Balls": manchurianBallsImage,
  "Sweet & Sour Fish": photo_1623689046286,
  "Chow Mein Chicken": photo_1769690507359,
  "Tofu Black Bean": premium_photo_1694860349260,
  "Prawn Chilli Basil": premium_photo_1661607229579,
  "Veg Dim Sum": photo_1756821753259,
  "Masala Dosa": photo_1589302168068,
  "Idli Sambar": photo_1632104667384,
  "Medu Vada": photo_1730191843435,
  "Rava Uttapam": istockphoto_1256957996,
  "Pongal": istockphoto_482366402,
  "Chettinad Chicken": photo_1545247181,
  "Kerala Fish Curry": photo_1620894580123,
  "Appam & Stew": istockphoto_1365183162,
  "Andhra Mutton Curry": photo_1589302168068,
  "Filter Coffee": photo_1589302168068,
  "Butter Chicken": photo_1589302168068,
  "Dal Makhani": photo_1589302168068,
  "Paneer Butter Masala": photo_1589302168068,
  "Rogan Josh": photo_1589302168068,
  "Chole Bhature": photo_1589302168068,
  "Tandoori Roti Basket": photo_1589302168068,
  "Palak Paneer": photo_1589647363585,
  "Lamb Vindaloo": photo_1589302168068,
  "Malai Kofta": photo_1589302168068,
  "Amritsari Chicken Curry": photo_1603496987351,
  "Gulab Jamun": photo_1595608010652,
  "Rasmalai": photo_1589302168068,
  "Molten Chocolate Cake": photo_1589302168068,
  "Tiramisu Classico": photo_1589302168068,
  "Cheesecake New York": photo_1589302168068,
  "Kulfi Falooda": photo_1589302168068,
  "Crème Brûlée": photo_1589302168068,
  "Gajar Halwa": photo_1589302168068,
  "Baklava": photo_1589302168068,
  "Chocolate Brownie": photo_1589302168068,
  "Mango Lassi": photo_1589302168068,
  "Masala Chai": photo_1589302168068,
  "Cold Coffee": photo_1589302168068,
  "Fresh Lime Soda": photo_1589302168068,
  "Watermelon Cooler": photo_1589302168068,
  "Virgin Mojito": photo_1589302168068,
  "Rose Milk": photo_1589302168068,
  "Iced Tea Peach": photo_1589302168068,
  "Fresh Orange Juice": photo_1589302168068,
  "Kulfi Malai": photo_1589302168068,
  "Belgian Chocolate": photo_1589302168068,
  "Vanilla Bean": photo_1589302168068,
  "Strawberry Sorbet": photo_1589302168068,
  "Pistachio Gelato": photo_1589302168068,
  "Mango Sorbet": photo_1589302168068,
  "Cookies & Cream": photo_1589302168068,
  "Salted Caramel": photo_1589302168068,
  "Butterscotch Nut": photo_1589302168068,
  "Coconut Sorbet": photo_1589302168068,
};

/**
 * Gallery image collection.
 * All gallery images explicitly imported for production bundling.
 */
export const GALLERY_IMAGES = {
  restaurant: [
    { src: interiorImage, alt: "Restaurant Interior" },
    { src: heroRestaurantImage, alt: "Hero Shot" },
    { src: bgReservationsImage, alt: "Reservations Area" },
  ],
  food: [
    { src: gallery1Image, alt: "Gallery 1" },
    { src: gallery2Image, alt: "Gallery 2" },
    { src: gallery3Image, alt: "Gallery 3" },
    { src: dish1Image, alt: "Signature Dish 1" },
    { src: dish2Image, alt: "Signature Dish 2" },
    { src: dish3Image, alt: "Signature Dish 3" },
    { src: dish4Image, alt: "Signature Dish 4" },
    { src: dish5Image, alt: "Signature Dish 5" },
    { src: dish6Image, alt: "Signature Dish 6" },
  ],
  chef: [
    { src: chefImage, alt: "Chef" },
  ],
  events: [
    { src: bgContactImage, alt: "Events" },
  ],
  kitchen: [
    { src: bgAboutImage, alt: "Kitchen" },
  ],
};

/**
 * Background images for sections.
 */
export const BG_IMAGES = {
  gallery: bgGalleryImage,
  contact: bgContactImage,
  about: bgAboutImage,
  menu: bgMenuImage,
  reservations: bgReservationsImage,
};

/**
 * Get image for a specific dish.
 * Falls back to a dish placeholder if not found.
 */
export function getDishImage(dishName: string): string {
  return DISH_IMAGES[dishName] || dish1Image;
}

/**
 * Get all gallery images flattened for display.
 */
export function getAllGalleryImages() {
  return [
    ...GALLERY_IMAGES.restaurant,
    ...GALLERY_IMAGES.food,
    ...GALLERY_IMAGES.chef,
    ...GALLERY_IMAGES.events,
    ...GALLERY_IMAGES.kitchen,
  ];
}
