...
if (fs.existsSync(wranglerPath)) {
  try {
    ...
    if (json.assets && json.assets.directory) {
      ...
      console.log(
        `[normalize-paths] Normalized wrangler.json assets directory: "${prevDir}" -> "${json.assets.directory}"`,
      );
    }
...
if (fs.existsSync(deployConfigPath)) {
  try {
    ...
    if (json.configPath) {
      ...
      console.log(
        `[normalize-paths] Normalized deploy configPath: "${prevConfigPath}" -> "${json.configPath}"`,
      );
    }
